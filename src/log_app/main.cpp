...
int main(int argc, char **argv)
{
    ...
    try {
        ...
        if (logger)
            file << "Connecting to the MQTT server..." << std::endl;
        ...
        if (!rsp.is_session_present()) {
            while (getline(topic_file, topic)) {
                ...
                if (logger)
                    file << "subscribed to topic: " << topic << std::endl;
            }
        }
        ...
        if (logger)
            file << "Connected and ready to log" << std::endl;
        ...
        while (true) {
            ...
            if (cli.try_consume_message(&msg)) {
                ...
                if (logger)
                    file << std::put_time(&tm, "%d-%m-%Y %H:%M") << " " << msg->get_topic() << " " << msg->get_payload() << std::endl;
            }
        }
